...
def get_map(MINOVERLAP, draw_plot, score_threhold=0.5, path='./map_out', csv=False):
    ...
    with open(RESULTS_FILES_PATH + "/results.txt", 'w') as results_file:
        ...
        # # 定义统计变量
        # sum_TP = 0  # 新增变量：用于累计所有类别的True Positives
        # sum_FP = 0  # 新增变量：用于累计所有类别的False Positives
        # sum_FN = 0  # 新增变量：用于累计所有类别的False Negatives
        sum_obj = 0 # 新增变量：用于累计所有类别的数量
        sum_P = 0  # 新增变量
        sum_R = 0  # 新增变量
        ...
        for class_index, class_name in enumerate(gt_classes):
            ...
            # if len(tp) > 0:
            #     sum_TP += tp[-1]
            #     sum_FN += gt_counter_per_class[class_name] - tp[-1]
            # else:
            #     sum_FN += gt_counter_per_class[class_name]
            # if len(fp) > 0:
            #     sum_FP += fp[-1]
            ...
            if len(prec)>0:
                ...
                print(text + "\t||\tscore_threhold=" + str(score_threhold) + " : " + "F1=0.00% ; Recall=0.00% ;"
                                                                                     "Precision=0.00%")
            #_______________________________________
            sum_P = prec[score_threhold_idx] * gt_counter_per_class[class_name] + sum_P
            sum_R = rec[score_threhold_idx] * gt_counter_per_class[class_name] + sum_R
            sum_obj = gt_counter_per_class[class_name] + sum_obj
            #_______________________________________
        ...
        total_precision = sum_P/sum_obj
        total_recall = sum_R/sum_obj
        total_F1 = 2 * total_precision * total_recall / (total_precision + total_recall)
    ...
    if csv:
        return mAP, total_precision, total_recall, total_F1
    return mAP, total_precision, total_recall, total_F1
